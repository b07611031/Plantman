...
weights = './models/yolov9/data/yolo-c-tomato9/weights/best.pt'
dnn = False
data = './models/yolov9/data/data.yaml'
half = False
imgsz = (640, 640)
...
def inference_leaflet_diagnosis_model(image_path):
    prediction = run(source=image_path, conf_thres=0.25, iou_thres=0.8, line_thickness=8, 
        save_txt=True, save_conf=True, name='linebot004', exist_ok=True)
    prediction =prediction[0].cpu().numpy()
    prediction = list(dict.fromkeys([id2label[round(i[5])] for i in prediction]))
    # save_path = save_paths[0]
    return prediction


# image_path = './static/default/diseases/Early_blight_early_stage.jpg'
